...
public class QuantumGateUIDynamic : MonoBehaviour
{
    ...
    public float itemRevealDelay = 0.15f; // seconds between reveals
    ...
    private IEnumerator AnimateResults(List<Gear> pulledGears)
    {
        ...
        foreach (var gear in pulledGears)
        {
            ...
            // Glow flash for each item
            if (rarityGlowImage != null && rarityColorGradient != null)
            {
                float normalized = Mathf.InverseLerp(0, 10, (int)gear.Rarity);
                Color glowColor = rarityColorGradient.Evaluate(normalized);
                StartCoroutine(FlashGlow(glowColor));
            }

            // Optional: play rarity-based sound
            // AudioManager.Play("Pull" + gear.Rarity.ToString());
