...
public class OfflineRewardManager : MonoBehaviour
{
    ...
    [Header("Settings")]
    [Tooltip("Maximum hours offline that will generate rewards (e.g. 12h cap like LoM)")]
    public float MaxOfflineHours = 12f;

    [Tooltip("Whether to automatically grant rewards on startup")]
    ...
    {
        string timeStr = DateTime.UtcNow.ToString("O"); // round-trip format
